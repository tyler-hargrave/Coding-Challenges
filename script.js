...
function range(x, y) {
 if (x > y) return "First argument must be less than second";
 result = [];
...
function charCount(x) {
 ...
 for (let i = 0; i < x.length; i++) {
  if (result[x[i]] === undefined) result[x[i]] = 0;
  result[x[i]]++;
 }
 return result;
}
...
function formatWithPadding(x, strPad, lenPad) {
 let strX = "" + x;
 for (let i = strX.length; i < lenPad; i++) {
  strX = strPad + strX;
 }
 return strX;
}
...
function isPalindrome(strX) {
 ...
 for (let i = 0; i < strX.length; i++) {
  j = strX.length - 1 - i;
  if (strX[i].toUpperCase() !== strX[j].toUpperCase()) return false;
 }
 return true;
}
/*-----------------------------------------------------------------
...
function hammingDistance(x, y) {
 ...
 let result = 0;
 for (let i = 0; i < x.length; i++) {
  if (x[i].toUpperCase() !== y[i].toUpperCase()) result++;
 }
 return result;
}
/*-----------------------------------------------------------------
...
function reduceArray(arr, func, value) {
 let i = 0;
 while (i < arr.length) {
...
function isPrime(x) {
 ...
 //Integer
 let maxDivisor = Math.floor(Math.sqrt(x));
...
function intersection(arrX, arrY) {
 result = [];
...
function balancedBracketsOLD(str) {
 let i = 0;
 ...
 let x = open.indexOf(str[0]);
 let j = str.lastIndexOf(close[x]);
 //Not found
 if (j === -1) return false;

 //Found - Search in between and outside
 return balancedBrackets(str.substring(1, j)) && balancedBrackets(str.substring(j + 1, str.length));
}
function balancedBrackets(s) {
 let stack = [];
 for (let i = 0; i < s.length; i++) {
  //Opening Brack - add to the "stack"
  if (s[i] === "(" || s[i] === "[" || s[i] === "{") {
   stack.push(s[i]);
  }
  //Closing bracket and no stack - thats a fail
  else if (stack.length === 0) {
   return false;
  }
  //Closing Bracket - We have a match - remove the element from the stack - its all good
  else if (
  ...
  //Something bad happend that is a fail
  else {
   return false;
  }
 }
...
function isWinningTicket(arr) {
 for (let q = 0; q < arr.length; q++) {
  let subArr = arr[q];
  let subResult = false;
  let subChar = String.fromCharCode(subArr[1]);
  for (let i = 0; i < subArr[0].length; i++) {
   if (subArr[0][i] === subChar) subResult = true;
  }
...
function toCamelCase(str) {
 ...
 str = str.replaceAll(/\W\w/g, function (txt) {
...
function gridTrip(coordinates, moves) {
 let directions = ["U", "D", "R", "L"];
 let regex = /[A-Z]/g;
 let found = moves.match(regex);
 let amounts = moves.split(regex);
 for (let i = 0; i < found.length; i++) {
  //X = 0 and Y = 1 as the coordinates
  let thisDirection = directions.indexOf(found[i]);
  let xy = 0;
  if (thisDirection > 1) {
   xy = 1;
   thisDirection -= 2;
...
function totalTaskTime(queue, cpus) {
 let cpuFinish = [];
 for (let i = 0; i < cpus; i++) cpuFinish.push(0);

 let currentTime = 0;
 for (i = 0; i < queue.length; i++) {
  currentTime = Math.min(...cpuFinish);
  selectedCPU = cpuFinish.indexOf(currentTime);
  cpuFinish[selectedCPU] += queue[i];
 }
 return Math.max(...cpuFinish);
}
